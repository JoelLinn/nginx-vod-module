	...
	media_parse_params_t* parse_params)
{
	...
	{
		vod_and_bits(tracks_mask[media_type], cur_source->tracks_mask[media_type], request_tracks_mask[media_type], MAX_TRACK_COUNT);
	}
	...
	media_generator_t* generator)
{
	...
	for (media_type = 0; media_type < MEDIA_TYPE_COUNT; media_type++)
	{
		vod_and_bits(track_mask_temp, parse_params->required_tracks_mask[media_type], generator->tracks_mask[media_type], MAX_TRACK_COUNT);
		if (vod_is_any_bit_set(track_mask_temp, MAX_TRACK_COUNT))
		{
			return 1;
		}
	}
...
ngx_http_vod_start_processing_media_file(ngx_http_vod_ctx_t *ctx)
{
	...
	if (ctx->request == NULL &&
		...
		vod_is_all_bits_set(cur_source->tracks_mask[MEDIA_TYPE_AUDIO], MAX_TRACK_COUNT) &&
		vod_is_all_bits_set(cur_source->tracks_mask[MEDIA_TYPE_VIDEO], MAX_TRACK_COUNT))
	{
...
ngx_http_vod_map_media_set_apply(ngx_http_vod_ctx_t *ctx, ngx_str_t* mapping, int* cache_index)
{
		...
		mapped_media_set.closed_captions == NULL)
	{
		...
		if (mapped_source->clip_from == 0 &&
			...
			vod_is_all_bits_set(mapped_source->tracks_mask[MEDIA_TYPE_AUDIO], MAX_TRACK_COUNT) &&
			vod_is_all_bits_set(mapped_source->tracks_mask[MEDIA_TYPE_VIDEO], MAX_TRACK_COUNT))
		{
...
ngx_http_vod_handler(ngx_http_request_t *r)
{
	...
	else
	{
		...
		vod_set_bits(request_params.tracks_mask[MEDIA_TYPE_VIDEO], MAX_TRACK_COUNT);
		vod_set_bits(request_params.tracks_mask[MEDIA_TYPE_AUDIO], MAX_TRACK_COUNT);
