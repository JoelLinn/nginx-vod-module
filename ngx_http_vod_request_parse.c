	...
	track_mask_t* result)
{
	...
	ngx_memzero(result, sizeof(track_mask_t) * MEDIA_TYPE_COUNT);
	...
	for (;;)
	{
		...
		if (stream_index == 0)
		{
			...
			vod_set_bits(result[media_type], MAX_TRACK_COUNT);
		}
		else if (stream_index > MAX_TRACK_COUNT)
		{
			vod_log_error(NGX_LOG_WARN, r->connection->log, 0,
				"ngx_http_vod_extract_track_tokens: the track index %i of type %i exceeds the maximum track count of %i",
				stream_index, media_type, MAX_TRACK_COUNT);
		}
		else
		{
	...
	request_params_t* result)
{
	...
	{
		vod_set_bits(default_tracks_mask, MAX_TRACK_COUNT);
	}
	else
	{
		vod_reset_bits(default_tracks_mask, MAX_TRACK_COUNT);
...
ngx_http_vod_parse_tracks_param(ngx_str_t* value, void* output, int offset)
{
	...
	ngx_memzero(tracks_mask, sizeof(track_mask_t) * MEDIA_TYPE_COUNT);
	...
	media_set_t* media_set)
{
	...
	for (i = 0; i < multi_uri.parts_count; i++)
	{
		...
		for (media_type = 0; media_type < MEDIA_TYPE_COUNT; media_type++)
		{
			vod_and_bits(track_mask_temp, cur_source->tracks_mask[media_type], request_params->tracks_mask[media_type], MAX_TRACK_COUNT);
