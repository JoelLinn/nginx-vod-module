...
def generate_ffmpeg_command(languages, max_tracks, output_filename):
	...
	for ident in languages.keys():
		c = np.append(c, ['-f', 'lavfi', '-i', 'flite=text="{}",aloop=20:size=3*8000'.format(pattern.sub('', languages[ident]))])
		langs = np.append(langs, ident)
		if len(langs) == max_tracks:
			break

	c = np.append(c, '-shortest')

	for i in range(len(langs)):
		c = np.append(c, ['-map', '{}:a'.format(i), '-metadata:s:a:{}'.format(i), 'language={}'.format(langs[i])])
	c = np.append(c, output_filename)
	return c
