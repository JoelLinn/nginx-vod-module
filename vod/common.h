...
#define vod_number_of_residual_bits(i) ((i) % vod_number_of_bits(uint64_t))
#define vod_array_length_for_bits(i) ((i) / vod_number_of_bits(uint64_t) + !!((i) % vod_number_of_bits(uint64_t)))
#define vod_is_bit_set(mask, index) (!!((mask)[(index) / vod_number_of_bits((mask)[0])] & ((uint64_t)1 << ((index) % vod_number_of_bits((mask)[0])))))
#define vod_set_bit(mask, index) ((mask)[(index) / vod_number_of_bits((mask)[0])] |= ((uint64_t)1 << ((index) % vod_number_of_bits((mask)[0]))))
#define vod_reset_bit(mask, index) ((mask)[(index) / vod_number_of_bits((mask)[0])] &= ~((uint64_t)1 << ((index) % vod_number_of_bits((mask)[0]))))
#define vod_set_bits(mask, max_bits) vod_memset((mask), 0xff, sizeof(uint64_t) * vod_array_length_for_bits(max_bits));
#define vod_reset_bits(mask, max_bits) vod_memzero((mask), sizeof(uint64_t) * vod_array_length_for_bits(max_bits));
	...
	uint32_t max_bits)
{
	...
	for (uint32_t i = 0; i < max_bits / vod_number_of_bits(uint64_t); i++)
	{
...
vod_get_lowest_bit_set(
	...
	uint32_t max_bits)
{
	for (uint32_t i = 0; max_bits / vod_number_of_bits(uint64_t); i++)
	{
		if (mask[i] != (uint64_t)0)
		{
			return i * vod_number_of_bits(uint64_t) + vod_get_trailing_zeroes64(mask[i]);
		}
	}
...
vod_and_bits(
	...
	uint32_t max_bits)
{
	for (uint32_t i = 0; i < vod_array_length_for_bits(max_bits); i++)
	{
		dst[i] = a[i] & b[i];
	}
}
