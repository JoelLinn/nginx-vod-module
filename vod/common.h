...
#define vod_is_bit_set(mask, index) (((mask)[(index) / vod_number_of_bits((mask)[0])] >> ((index) % vod_number_of_bits((mask)[0]))) & 1)
	...
	uint32_t max_bits)
{
	...
	for (i = 0; i < max_bits / vod_number_of_bits(uint64_t); i++)
	{
...
{
	uint32_t i;
	for (i = 0; max_bits / vod_number_of_bits(uint64_t); i++)
	{
		if (mask[i] != (uint64_t)0)
		{
