...
static u_char*
manifest_utils_write_bitmask(u_char* p, size_t* bitmask, size_t* mask_temp, uint32_t max_bits, u_char letter)
{
	uint32_t i;
	uint32_t count = vod_get_number_of_set_bits_in_mask(mask_temp, max_bits);

	vod_memcpy(mask_temp, bitmask, sizeof(size_t) * vod_array_length_for_bits(max_bits));
